...
from .Optimal_Assortment import getOptimalAssortment as lp_opt
from .draw import gen_figure
...
def run(D=5, K=6, B=5, NN=50, debug=False):
    ...
    # 2d-array
    Theta_g_p = (2*np.random.normal(0, 1, size=(D, K)) -
                 np.random.uniform(0, 1, size=(D, K)))/np.sqrt(D*K)
    ...
    def getOptimalAssortment(Theta, Nx, x):
        ...
        for i in range(nx):
            ...
            rx[i] = r[Nx[i]-1]
        opt_as = lp_opt(n=nx, w=wx, r=rx, B=B, log=False)
    ...
    def PAO_TS_exp(T, r):
        ...
        for t in range(1, T+1):
            ...
            if len(H_TS_p) == 0:
                ...
                Theta_ts_p = Generate_theta_p(
                    *zip(*H_TS_p), N=len(N), K=K, prod_f=prod_f)

            opt_as_ts_p = getOptimalAssortment(
                getInferredTheta(Theta_ts_p), Nx, x)
            ...
            print(np.max(np.abs(Theta_ts_np-Theta_g_np).flatten()),
                  np.max(np.abs(getInferredTheta(Theta_ts_p)-Theta_g_np).flatten()))
            print(sum(reward_np)-sum(reward_ora),
                  sum(reward_p)-sum(reward_ora))
